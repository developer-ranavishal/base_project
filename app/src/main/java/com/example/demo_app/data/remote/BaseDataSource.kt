...
import androidx.lifecycle.liveData
import app.beYou.data.remote.data.BaseResponseGeneric
...
abstract class BaseDataSource {
    ...
    protected suspend fun <T> getResult(tag: String, call: suspend () -> Response<BaseResponseGeneric<T>>): Resource<BaseResponseGeneric<T>> {
        try {
            ...
            e.printStackTrace()
            logD("account", e.message.toString())
    ...
    fun <T> performOperation(tag: String, networkCall: suspend () -> Resource<T>): LiveData<Resource<T?>> = liveData(
        Dispatchers.IO) {
        try {
            emit(Resource.loading(tag, data = null))
            val responseStatus = networkCall.invoke()
